...
logging.basicConfig(
    ...
    level=logging.INFO,
    stream=sys.stdout # Render এর কনসোলে লগ দেখানোর জন্য
)
...
try:
    # অ্যাডমিন আইডি অবশ্যই পূর্ণ সংখ্যা (integer) হতে হবে
...
# WARNING: এটি অস্থায়ী মেমরি। বট বন্ধ হলে বা রিস্টার্ট হলে ডেটা মুছে যাবে।
user_data = {} 
...
async def check_channel_member(context: ContextTypes.DEFAULT_TYPE, user_id):
    ...
    try:
        # @ দিয়ে শুরু হওয়া চ্যানেল ইউজারনেম ব্যবহার করতে হবে
        member = await context.bot.get_chat_member(CHANNEL_USERNAME, user_id)
        return member.status in ['member', 'administrator', 'creator']
    except Exception as e:
        # যদি চ্যানেল খুঁজে না পাওয়া যায় বা ইউজার ব্লক করে, তবে এখানে ধরা হবে
...
def get_main_keyboard(user_id):
    ...
    keyboard = [
        ...
        # এখানে Adsterra Smart Link ব্যবহার করুন 
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if context.args and context.args[0].startswith('ref'):
        try:
            # রেফারেল লিংক থেকে রেফারার আইডি বের করা
    ...
    if user_id not in user_data:
        ...
        if referrer_id and referrer_id in user_data:
            ...
            try:
                ...
                # যদি রেফারার বটটি ব্লক করে থাকে
                pass 
    ...
    if update.callback_query:
        # কলব্যাক কোয়েরির মাধ্যমে এলে মেসেজ এডিট করা
        ...
        # /start কমান্ড বা সাধারণ মেসেজের মাধ্যমে এলে রিপ্লাই করা
...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if query.data == 'check_join':
        ...
        if is_member_after_check:
            ...
            # /start ফাংশন কল করা যাতে Welcome মেসেজ ও কীবোর্ড আসে
            ...
            if user_id in user_data:
                ...
                if referrer_id and referrer_id in user_data:
                    ...
                    try:
                        ...
                        pass # ব্লক করা ইউজারকে মেসেজ পাঠানো যাবে না
                ...
                # যদি কোনো কারণে ইউজার ডেটা না থাকে
                message = "⛔ আপনার ডেটা খুঁজে পাওয়া যায়নি। দয়া করে /start লিখে বটটি আবার শুরু করুন।"
    ...
    # --- উইথড্রয়াল ---
        ...
        if current_points >= MIN_WITHDRAW_POINTS:
            # ✅ উইথড্রয়াল মেসেজ: ইউজারকে পেমেন্ট ডিটেলস পাঠাতে বলা
            message = (
...
async def message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        try:
            ...
            if target_id in user_data:
                ...
                try:
                    ...
                    pass # টার্গেট ইউজার বট ব্লক করেছে
            else:
    ...
    # --- সাধারণ মেসেজ (উইথড্রয়াল রিকোয়েস্ট) ---
        ...
        # নিশ্চিত করুন যে ইউজারটি উইথড্রয়াল রিকোয়েস্ট পাঠাচ্ছেন কিনা
        if user_id in user_data and user_data[user_id].get('points', 0) >= MIN_WITHDRAW_POINTS:
            ...
            await context.bot.send_message(
                ...
                text=f"💸 **নতুন উইথড্রয়াল রিকোয়েস্ট!**\n"
                    f"ইউজার ID: `{user_id}`\n"
                    f"বর্তমান পয়েন্ট: {user_data[user_id].get('points', 0)}\n"
                    f"মেসেজ: {text}"
            )

            await update.message.reply_text(
                "আপনার উইথড্রয়াল রিকোয়েস্ট অ্যাডমিনকে পাঠানো হয়েছে। এটি ম্যানুয়ালি দেখা হচ্ছে।\n"
                "অন্যান্য অপশনের জন্য মেনু ব্যবহার করুন:",
                reply_markup=get_main_keyboard(user_id)
            )
        else:
            # যদি ইউজার উইথড্র করার জন্য যথেষ্ট পয়েন্ট না রেখেই মেসেজ করে
            await update.message.reply_text(
                "আপনার মেসেজটি (উইথড্রয়াল রিকোয়েস্ট নয়) অ্যাডমিনকে পাঠানো হয়েছে। মেনু ব্যবহার করে কাজ শুরু করুন:",
                reply_markup=get_main_keyboard(user_id)
            )
...
def main() -> None:
    ...
    if not TELEGRAM_BOT_TOKEN:
        logger.error("❌ TELEGRAM_BOT_TOKEN এনভায়রনমেন্ট ভেরিয়েবলে পাওয়া যায়নি।")
        # sys.exit(1)
        return

    if ADMIN_USER_ID == 12345678:
         logger.warning("⚠️ WARNING: ADMIN_USER_ID পরিবর্তন করা হয়নি। এটি অবশ্যই আপনার নিজস্ব আইডি হতে হবে।")
    ...
    # সাধারণ টেক্সট মেসেজ, কিন্তু কমান্ড ছাড়া
    application.add_handler(MessageHandler(filters.TEXT & (~filters.COMMAND), message_handler)) 

    print("✅ Smart Earn Bot Running... Check console for logs.")
    
    # ⭐ Render বা অন্যান্য Non-Webhook পরিবেশে ব্যবহারের জন্য
